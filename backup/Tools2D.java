...
class Tools2D {
	...
	static boolean insideTriangle(Point2D A, Point2D B, Point2D C, Point2D P) // ABC is assumed to be counter-clockwise
	{
	...
	static boolean insidePolygon(Point2D p, Point2D[] pol) {
		...
		for (int i = 0; i < n; i++) {
			if (pol[j].y <= y && y < pol[i].y && Tools2D.area2(pol[j], pol[i], p) > 0
					|| pol[i].y <= y && pol[j].y > y && Tools2D.area2(pol[i], pol[j], p) > 0)
				b = !b;
			j = i;
		}
		return b;
